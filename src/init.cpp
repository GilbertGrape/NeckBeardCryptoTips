...
void Shutdown()
{
    ...
    RenameThread("NeckBeardCryptoTips-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: NeckBeardCryptoTips.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: NeckBeardCryptoTipsd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeNeckBeardCryptoTipsamount=<n> " + _("Keep N NeckBeardCryptoTips anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. NeckBeardCryptoTips is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. NeckBeardCryptoTips is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("NeckBeardCryptoTips version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "NeckBeardCryptoTips server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of NeckBeardCryptoTips") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart NeckBeardCryptoTips to complete") << "\n";
    ...
    nAnonymizeNeckBeardCryptoTipsAmount = GetArg("-anonymizeNeckBeardCryptoTipsamount", 0);
    if(nAnonymizeNeckBeardCryptoTipsAmount > 999999) nAnonymizeNeckBeardCryptoTipsAmount = 999999;
    if(nAnonymizeNeckBeardCryptoTipsAmount < 2) nAnonymizeNeckBeardCryptoTipsAmount = 2;
    ...
    LogPrintf("Anonymize NeckBeardCryptoTips Amount %d\n", nAnonymizeNeckBeardCryptoTipsAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1NeckBeardCryptoTips+1000 == (.1NeckBeardCryptoTips+100)*10
       10NeckBeardCryptoTips+10000 == (1NeckBeardCryptoTips+1000)*10
    */

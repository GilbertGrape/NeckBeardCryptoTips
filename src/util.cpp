...
int nAnonymizeNeckBeardCryptoTipsAmount = 1000;
...
#else
    const char* pszModule = "NeckBeardCryptoTips";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NeckBeardCryptoTips
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NeckBeardCryptoTips
    // Mac: ~/Library/Application Support/NeckBeardCryptoTips
    // Unix: ~/.NeckBeardCryptoTips
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NeckBeardCryptoTips";
#else
    fs::path pathRet;
    ...
    return pathRet / "NeckBeardCryptoTips";
#else
    // Unix
    return pathRet / ".NeckBeardCryptoTips";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "NeckBeardCryptoTips.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "NeckBeardCryptoTipsd.pid"));

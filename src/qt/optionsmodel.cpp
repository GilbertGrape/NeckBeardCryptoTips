...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeNeckBeardCryptoTipsAmount"))
        settings.setValue("nAnonymizeNeckBeardCryptoTipsAmount", 1000);
    nAnonymizeNeckBeardCryptoTipsAmount = settings.value("nAnonymizeNeckBeardCryptoTipsAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeNeckBeardCryptoTipsAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeNeckBeardCryptoTipsAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeNeckBeardCryptoTipsAmount:
            return QVariant(nAnonymizeNeckBeardCryptoTipsAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeNeckBeardCryptoTipsAmount:
            nAnonymizeNeckBeardCryptoTipsAmount = value.toInt();
            settings.setValue("nAnonymizeNeckBeardCryptoTipsAmount", nAnonymizeNeckBeardCryptoTipsAmount);
            emit AnonymizeNeckBeardCryptoTipsAmountChanged(nAnonymizeNeckBeardCryptoTipsAmount);
            break;
        default:
            break;
        }

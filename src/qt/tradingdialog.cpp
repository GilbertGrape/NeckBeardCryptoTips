    ...
    model(0)
{
    ...
    ui->NeckBeardCryptoTipsAvailableLabel->setTextFormat(Qt::RichText);
    ...
    ui->BittrexNeckBeardCryptoTipsLabel->setTextFormat(Qt::RichText);
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "NBCT(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "NBCT(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    /*OrderBook Table Init*/

    /*Market History Table Init*/
    ui->MarketHistoryTable->setColumnCount(5);
    ui->MarketHistoryTable->verticalHeader()->setVisible(false);
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(NBCT)"<<"TOTAL COST(BTC");
...
void tradingDialog::UpdaterFunction(){
    //NeckBeardCryptoTipsst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://bittrex.com/api/v1.1/public/GetMarketSummary?market=btc-NBCT");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://bittrex.com/api/v1.1/public/getorderbook?market=BTC-NBCT&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://bittrex.com/api/v1.1/public/getmarkethistory?market=BTC-NBCT&count=100");
      return Response;
}
...
QString tradingDialog::BuyNeckBeardCryptoTips(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://bittrex.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-NBCT&quantity=";
...
QString tradingDialog::SellNeckBeardCryptoTips(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://bittrex.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-NBCT";
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getdepositaddress?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=NBCT";
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-NBCT&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>NBCT Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> NBCT");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    double NeckBeardCryptoTipsSupply = 0;
    double NeckBeardCryptoTipsDemand = 0;
    double BtcSupply = 0;
    double BtcDemand = 0;
    ...
    ui->NeckBeardCryptoTipsSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(NeckBeardCryptoTipsSupply,'i',8) + "</span><b> NBCT</b>");
    ...
    ui->NeckBeardCryptoTipsDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(NeckBeardCryptoTipsDemand,'i',8) + "</span><b> NBCT</b>");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
                case 0:    //buy tab is active
                    ...
                    Response2 = GetBalance("NBCT");
                    Response3 = GetOrderBook();

                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BtcAvailableLabel, *ui->NeckBeardCryptoTipsAvailableLabel, Response, Response2);
                    }
                ...
                case 1: //Cross send tab active
                    Response = GetBalance("NBCT");
                    Response2 = GetBalance("BTC");
                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BittrexNeckBeardCryptoTipsLabel, *ui->BittrexBTCLabel, Response, Response2);
                    }
                ...
                case 5://show balance tab
                    ...
                    Response = GetBalance("NBCT");
                    if(Response.size() > 0 && Response != "Error"){
                        DisplayBalance(*ui->NeckBeardCryptoTipsBalanceLabel,*ui->NeckBeardCryptoTipsAvailableLabel_2,*ui->NeckBeardCryptoTipsPendingLabel, QString::fromUtf8("NBCT"),Response);
                    }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputNeckBeardCryptoTips->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("NBCT");
    ...
    double AvailableNeckBeardCryptoTips = BalanceObject["Available"].toDouble();
    ...
    QString DumpStr = "";
    QString TotalStr = "";
    if ( Qty < AvailableNeckBeardCryptoTips )
    {
...
{
    //calculate amount of BTC that can be gained from selling NBCT available balance
    QString responseA = GetBalance("NBCT");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONObject(responseA);

    double AvailableNeckBeardCryptoTips = ResultObject["Available"].toDouble();

    ui->UnitsInputNeckBeardCryptoTips->setText(str.number(AvailableNeckBeardCryptoTips,'i',8));
}
...
{
    double Quantity = ui->BittrexNeckBeardCryptoTipsLabel->text().toDouble();
    double Received = 0;
    double Qty = 0;
    double Price = 0;
    QString buyorders = GetOrderBook();
...
void tradingDialog::on_Withdraw_Max_Amount_clicked()
{
    ...
    ui->WithdrawUnitsInput->setText(str.number(AvailableNeckBeardCryptoTips,'i',8));
}
...
void tradingDialog::on_BuyNeckBeardCryptoTips_clicked()
{
    double Rate;
    double Quantity;
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  BuyNeckBeardCryptoTips(Order,Quantity,Rate);
...
void tradingDialog::on_SellNeckBeardCryptoTipsBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputNeckBeardCryptoTips->text().toDouble();
    ...
    QString Msg = "Are you sure you want to Sell ";
            Msg += ui->UnitsInputNeckBeardCryptoTips->text();
            Msg += " NBCT @ ";
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  SellNeckBeardCryptoTips(Order,Quantity,Rate);
        QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
        QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
    ...
    if(!ctx.isValid())
    {
        ...
        foreach (const QJsonValue & value, BuyArray)
        {
            ...
            if ( ((Quantity / x) - y) > 0 )
            {
                ...
                QString SellResponse = SellNeckBeardCryptoTips(Order,y,x);
                ...
                QString SellResponse = SellNeckBeardCryptoTips(Order,(Quantity / x),x);
                QJsonDocument SelljsonResponse = QJsonDocument::fromJson(SellResponse.toUtf8());          //get json from str.
                QJsonObject SellResponseObject = SelljsonResponse.object();                              //get json obj
                ...
                if (SellResponseObject["success"].toBool() == false){
                    ...
                    if (ResponseObject["success"].toBool() == false){
                        ...
                        if (ResponseObject["success"].toBool() == false){
                            ...
                            QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" NBCT for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                        }
                    } else if (ResponseObject["success"].toBool() == true){
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" NBCT for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
...
void tradingDialog::on_UnitsInputNeckBeardCryptoTips_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}

...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("NeckBeardCryptoTips"))
        return false;
...
{
    // Convert NeckBeardCryptoTips:// to NeckBeardCryptoTips:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("NeckBeardCryptoTips://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "NeckBeardCryptoTips:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "NeckBeardCryptoTips.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for NeckBeardCryptoTips.lnk
...
{
    return GetAutostartDir() / "NeckBeardCryptoTips.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a NeckBeardCryptoTips.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=NeckBeardCryptoTips\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the NeckBeardCryptoTips app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add NeckBeardCryptoTips app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("NeckBeardCryptoTips-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  NeckBeardCryptoTips-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("NeckBeardCryptoTips-Qt"));

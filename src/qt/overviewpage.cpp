...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeNeckBeardCryptoTipsAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeNeckBeardCryptoTipsAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeNeckBeardCryptoTipsAmount = strAnonymizeNeckBeardCryptoTipsAmount.remove(strAnonymizeNeckBeardCryptoTipsAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNeckBeardCryptoTipsAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeNeckBeardCryptoTipsAmount*COIN) nMaxToAnonymize = nAnonymizeNeckBeardCryptoTipsAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeNeckBeardCryptoTipsAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeNeckBeardCryptoTipsAmount));
        strAnonymizeNeckBeardCryptoTipsAmount = strAnonymizeNeckBeardCryptoTipsAmount.remove(strAnonymizeNeckBeardCryptoTipsAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNeckBeardCryptoTipsAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeNeckBeardCryptoTipsAmount == 0){

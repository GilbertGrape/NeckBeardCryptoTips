...
//
// Bootup the masternode, look for a 8000 NBCT input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in NeckBeardCryptoTips.conf */
...
{
    CNeckBeardCryptoTipscoinAddress address(collateralAddress);

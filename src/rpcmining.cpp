...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "NeckBeardCryptoTips is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "NeckBeardCryptoTips is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "NeckBeardCryptoTips is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "NeckBeardCryptoTips is downloading blocks...");
